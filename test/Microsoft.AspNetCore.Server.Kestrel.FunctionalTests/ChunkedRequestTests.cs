        ...
        public async Task ClosingConnectionMidChunkPrefixThrows(ListenOptions listenOptions)
        {
            ...
            }, testContext, listenOptions))
            {
                using (var connection = server.CreateConnection())
                {
                    ...
                    await readStartedTcs.Task.TimeoutAfter(TimeSpan.FromSeconds(30));

                    connection.Socket.Shutdown(SocketShutdown.Send);

                    await connection.ReceiveEnd();

                    var badReqEx = await exTcs.Task.TimeoutAfter(TimeSpan.FromSeconds(30));
                    var expectedExMessage = BadHttpRequestException.GetException(RequestRejectionReason.UnexpectedEndOfRequestContent).Message;
                    Assert.Equal(expectedExMessage, badReqEx.Message);
                }
            }
        }
    }
