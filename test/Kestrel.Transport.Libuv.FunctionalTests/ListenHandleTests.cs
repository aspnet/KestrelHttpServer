...
namespace Microsoft.AspNetCore.Server.Kestrel.FunctionalTests
{
    ...
    public class ListenHandleTests : LoggedTest
    {
        ...
        private static readonly Socket _listenSocket = new Socket(AddressFamily.InterNetwork, SocketType.Stream, ProtocolType.Tcp);
        ...
        public async Task CanListenToOpenTcpSocketHandle()
        {
            ...
            using (var server = new TestServer(_ => Task.CompletedTask, new TestServiceContext(LoggerFactory), new ListenOptions((ulong)_listenSocket.Handle)))
            {
                using (var connection = new TestConnection(((IPEndPoint)_listenSocket.LocalEndPoint).Port))
                {
