...
namespace Microsoft.AspNetCore.Server.Kestrel.Core.Internal.Http
{
    public static class PipelineExtensions
    {
        ...
        private unsafe static void EncodeAsciiCharsToBytes(char* input, byte* output, int length)
        {
            ...
            do
            {
                if (!Vector.IsHardwareAccelerated || input > end - Vector<ushort>.Count * 2)
                {
                    length = (int)(end - input);
                    int i = 0;
                ...
                do
                {
                    // Convert 2 char vectors to 1 byte Vector at a time
                    ...
                    input += Vector<ushort>.Count * 2;
                    output += Vector<byte>.Count;

                    // Loop while remaining double Vector sizes
                } while (input <= end - Vector<ushort>.Count * 2);
