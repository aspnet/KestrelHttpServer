...
namespace Microsoft.AspNet.Server.Kestrel.Http
{
    public class SocketOutput : ISocketOutput
    {
        ...
        public SocketOutput(MemoryPool memory, KestrelThread thread, UvStreamHandle socket, long connectionId, IKestrelTrace log)
        {
            _thread = thread;
            _socket = socket;
            ...
            bool socketDisconnect = false)
        {
            ...
            if (inputLength > 0)
            {
                ...
                while (remaining > 0)
                {
                    if (memoryBlock == null)
                    {
                        memoryBlock = _memory.Lease(MemoryPool.NativeBlockSize);
        ...
        {
            public ArraySegment<MemoryPoolBlock> Data;
